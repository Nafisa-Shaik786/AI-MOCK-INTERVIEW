...
const DSADashboard = () => {
  ...
  console.log("Backend URL:", BASE_URL); // ✅ Log to check VITE variable
  ...
  const handleStartCoding = async () => {
    ...
    console.log("Sending to backend:", { role, difficulty });

    try {
      const response = await fetch(`${BASE_URL}/api/dsa/generate-question`, {
        method: "POST",
      ...
      if (data.question) {
        localStorage.setItem("dsa-question", data.question);
        navigate("/code-editor");
      } else {
        alert(data.message || "Failed to get question.");
      }
    } catch (err) {
